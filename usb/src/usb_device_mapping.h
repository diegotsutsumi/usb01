...
#define USB_DEVICE_Detach( handle )     DRV_USB_DEVICE_Detach( ((USB_DEVICE_OBJ *)handle)->usbCDHandle)
...
#if (USB_DEVICE_SOF_EVENT_ENABLE)
    ...
    #define _USB_DEVICE_EndpointMutexCreate(x)       OSAL_MUTEX_Create(&(x->mutexEndpointIRP))
    #define _USB_DEVICE_EndpointMutexDelete(x)       OSAL_MUTEX_Delete(&(x->mutexEndpointIRP))
#else
