...
)
{
    ...
    switch (eventType)
    {
        ...
        //    DRV_USB_HOST_Suspend ( referenceHandle );
            /* Get device information */
            deviceInfo = &(gUSBHostObj[referenceHandle].attachedDevicesList[0]);
            
            if(timerRunning)                                 //------------------------
            {                                                //----Tsutsumi's Patch----
                USB_HOST_OBJ * hcObj = &(gUSBHostObj[0]);    //------------------------
                SYS_TMR_ObjectDelete(hcObj->sysTmrHandle);   //------------------------
            }
...
/*Start of Tsutsumi's Patch*/
void USB_HOST_SetEnumCompleted(EnumCompletedHandle callback)
{
    gUSBHostObj[0].enumCompHandle = callback;
}
/*End of Tsutsumi's Patch*/
                ...
                case USB_HOST_DEVICE_SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE:
                    ...
                    switch ( enumerateDeviceInfo->deviceState & USB_HOST_DEVICE_SUBSUBSTATE_MASK )
                    {
                        case USB_HOST_DEVICE_SUB_SUBSTATE_SEND_GET_DEVICE_DESCRIPTOR_SIZE:
                            ...
                            /* Fill control IRP. */
                            enumerateDeviceInfo->controlIrp.data = (int8_t *) &( enumerateDeviceInfo->deviceDescriptor );
                    ...
                    switch ( enumerateDeviceInfo->deviceState & USB_HOST_DEVICE_SUBSUBSTATE_MASK )
                    {
                        ...
                        case USB_HOST_DEVICE_SUB_SUBSTATE_GET_CONFIG_DESCRIPTOR_COMPLETE:
                            ...
                            if ( enumerateDeviceInfo->controlIrp.status == USB_HOST_IRP_STATUS_COMPLETED )
                            {
                                ...
                                _USB_HOST_ParseConfigDescriptor( enumerateDeviceInfo->rawBuffer,
                                ...
                                if( driverFoundStatus == true )
                                {
                                    ...
                                    enumerateDeviceInfo->activeConfigurationNumber = 0x01; //Tsutsumi's Patch
                                    enumerateDeviceInfo->deviceState =
                                        USB_HOST_DEVICE_STATE_CONFIGURING |
                                        USB_HOST_DEVICE_SUBSTATE_GET_CONFIG_DESCRIPTOR|
                                ...
                                else
                                {
                                    ...
                                    {
                                        enumerateDeviceInfo->driver->initializeDeviceDriver(
                                                    enumerateDeviceInfo->deviceId, &( enumerateDeviceInfo->instanceNumber ), //Tsutsumi's Patch
                                                    enumerateDeviceInfo->speed);
                    ...
                    switch ( enumerateDeviceInfo->deviceState & USB_HOST_DEVICE_SUBSUBSTATE_MASK )
                    {
                        ...
                        case USB_HOST_DEVICE_SUB_SUBSTATE_SET_CONFIGURATION_COMPLETE:
                            ...
                            if (SYS_TMR_DelayStatusGet(hcObj->sysTmrHandle))
                            {
                                ...
                                gUSBHostObj[0].enumCompHandle(); //Tsutsumi's Patch
...
uint8_t USB_HOST_GetVendorIndex()
{
    USB_HOST_DEVICE_INFO *device;
    uint8_t count;
    USB_HOST_TARGET_PERIPHERAL_LIST *driverInfo;
    uint8_t hostIndex;
    USB_HOST_OBJ *hcObj;
    ...
    device = &(hcObj->attachedDevicesList[0]);
    ...
    for( count = 0 ; count < hcObj->nTPLEntries ; count++ )
     {
        ...
        if( driverInfo->tplFlag == TPL_FLAG_VID_PID  )
        {
            ...
            if (( driverInfo->device.idVendor == device->deviceDescriptor.idVendor) &&
                    ( driverInfo->device.idProduct == device->deviceDescriptor.idProduct ))
            {
                device->driverType = USB_HOST_VENDOR_DEVICE_DRIVER;
                device->driver = driverInfo->classDriver;
                return count;
            }
        }
