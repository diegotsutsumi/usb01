...
/* Device layer instance objects.
  ...
  Summary:
    Holds all of the varaiables required by the instance.
...
static USB_DEVICE_CLIENT_STRUCT usbDeviceClients[USB_DEVICE_MAX_CLIENTS];
...
/******************************************************************************
  ...
  Description:
    This function internally calls all USB Device layer function driver tasks.

  Remarks:
    None.
*/

void _USB_DEVICE_MAKE_NAME(Tasks)( void )
{
    ...
    // Copy event (work on the copy, sice it is never known when an interrupt
    // occurrs and updates the event.
    event = usbDeviceInstance.event;

   // if( event == USB_DEVICE_EVENT_CONFIGURED )
    //{
        // Initialize function drivers for the new configuration.
      //  _USB_DEVICE_InitializeFunctionDrivers( usbDeviceThisInstance->activeConfiguration,
    ...
    if( (event == USB_DEVICE_EVENT_RESET) || (event == USB_DEVICE_EVENT_DECONFIGURED)
    ...
    if(usbDeviceInstance.usbDeviceState == USB_DEVICE_STATE_CONFIGURED)
    {
        ...
        {
            funcDriverIndex = usbDeviceInstance.functionDriver[count].funcDriverIndex;

            if(usbDeviceInstance.functionDriver[count].driver->tasks != NULL)
            {
                // Call function driver tasks
                usbDeviceInstance.functionDriver[count].driver->tasks(funcDriverIndex);
            }
        }
    }
...
/******************************************************************************
  ...
  Description:
    This function initializes the required USB device layer state machines.

  Remarks:
    None.
*/
void _USB_DEVICE_MAKE_NAME(Initialize)( const SYS_MODULE_INIT * const initData )
{
    ...
    usbDeviceInstance.driverHandle = DRV_USB_Open( deviceInit->usbDriverIndex, DRV_IO_INTENT_NONBLOCKING|DRV_IO_INTENT_READWRITE);
    SYS_ASSERT( ( DRV_HANDLE_INVALID != usbDeviceInstance.driverHandle ),"USB Device Layer: Bad handle returned from driver");
    
    
    // Register a callback with the driver.
    DRV_USB_Device_EventCallBackSet(usbDeviceInstance.driverHandle, 
                                            (void*)&usbDeviceInstance, _USB_DEVICE_EventHandler);
    
       
    // Setup the In pipe
    DRV_USB_Device_SetupPacketBufferAssign( usbDeviceInstance.driverHandle ,
                                       usbDeviceInstance.setupPacketBuffer );   
...
DRV_HANDLE _USB_DEVICE_MAKE_NAME(Open)( const DRV_IO_INTENT intent )
{
    uint8_t i;
	...
	for(i = 0; i < USB_DEVICE_MAX_CLIENTS; i++)
    {   // Check for a free handle.
        if(usbDeviceClients[i].clientState == DRV_CLIENT_STATUS_CLOSED)
        {
            // Ready the client.
            usbDeviceClients[i].clientState = DRV_CLIENT_STATUS_READY;
            retValue = (DRV_HANDLE)&usbDeviceClients[i];
        }    
    }  
...
void _USB_DEVICE_MAKE_NAME(Close)(DRV_HANDLE hClient )
{
...
DRV_CLIENT_STATUS _USB_DEVICE_MAKE_NAME(ClientStatus)( DRV_HANDLE hHandle )
{
...
/* Function:
  ...
  Description:
    This routine provides the current status of the USB device layer.
...
SYS_STATUS _USB_DEVICE_MAKE_NAME(Status)( void )
{
...
/******************************************************************************
  ...
  Parameters:
    hHandle           - Device layer handle that is returned from USB_DEVICE_Open

    callBackFunc      - Call back function that should be called in case of an event
   
  Returns:
    If successful, this function returns the error status "USB_ERROR_OK".
*/

USB_ERROR_STATUS USB_DEVICE_EventCallBackSet(DRV_HANDLE hHandle, const USB_DEVICE_CALLBACK callBackFunc)
{
...
/******************************************************************************
  Function:
    DRV_USB_DEVICE_XFER_STATUS USB_DEVICE_ControlTransferRequest
    (SYS_MODULE_INDEX usbDevIndex, uint8_t *controlDataBuffer, 
    uint16_t controlDataCount, DRV_USB_XFER_TYPE xferType, void *hClient, 
...
DRV_USB_XFER_HANDLE USB_DEVICE_ControlTransferRequest(SYS_MODULE_INDEX usbDevIndex,
        uint8_t *controlDataBuffer, uint16_t controlDataCount, DRV_USB_XFER_TYPE xferType, void *hClient, 
        const DRV_USB_XFER_COMPLETE_CALLBACK controlTxStatusCallback)
{
    ...
    return(DRV_USB_Device_TransferRequest( usbDeviceInstance.ctrlEpInHandle, 
                                          ...
                                          controlDataCount, false, hClient, controlTxStatusCallback));
}
...
DRV_USB_DEVICE_XFER_STATUS USB_DEVICE_ControlTransferStatusGet( DRV_USB_XFER_HANDLE hTransfer )
{
...
/******************************************************************************
  ...
  Description:
    This function stalls the TX/RX control pipe. The parameter "pipeDirection" specifies
    the direction of the pipe.
...
/******************************************************************************
  ...
  Description:
    This function will get the current active USB configuration. 
...
uint16_t USB_DEVICE_GetConfigurationValue(DRV_HANDLE hHandle)  
{
    ...
    index = devClientHandle->deviceLayerIndex;
...
/******************************************************************************
  ...
  Description:
    This function returns the speed on which the device was enumerated.
...
void _USB_DEVICE_MAKE_NAME(Reinitialize) ( const SYS_MODULE_INIT * const init )
{
...
void _USB_DEVICE_MAKE_NAME(Deinitialize)()
{
    uint8_t client;
    ...
    for( client = 0; client < USB_DEVICE_MAX_CLIENTS; client++ )
    {
        usbDeviceClients[client].clientState = DRV_CLIENT_STATUS_CLOSED ;        
    }       
}
            ...
            DRV_USB_EVENT eventType, DRV_USB_EVENT_DATA * eventData )
{
    ...
    switch(eventType)
    {
		case USB_DEVICE_RESET:
            ...
            usbDeviceInstance.ctrlEpInHandle =
                     DRV_USB_Device_PipeSetup(usbDeviceInstance.driverHandle, 0,
                                                USB_EP_TX_RX, 64, 1, USB_CONTROL_PIPE);
        ...
        case USB_REQUEST_SET_CONFIGURATION:
            ...
            else
            {
                ...
                if( usbDeviceInstance.activeConfiguration != setupPkt->wValue)
                {
                    ...
                    if( _USB_DEVICE_InitializeFunctionDrivers( setupPkt->wValue ) 
                            == USB_ERROR_OK )
                    {
...
static void _USB_DEVICE_ProcessOtherRequests( SETUP_PKT* setupPkt )
{
    ...
    if( setupPkt->bmRequestType & SETUP_RECIPIENT_INTERFACE )
    {
        ...
        for(lCount = 0; lCount < usbDeviceInstance.functionDriverCount; lCount++)
        {
            ...
            {
                if(lfunctionDriverPtr[lCount].driver->checkInterfaceNumber( lfunctionDriverPtr[lCount].funcDriverIndex, interfaceNumber ) == USB_ERROR_OK)
                {
                    ...
                    {
                        lfunctionDriverPtr[lCount].driver->setupPacketHandler
        				                	(lfunctionDriverPtr[lCount].funcDriverIndex, setupPkt); 
    ...
    else if( setupPkt->bmRequestType & SETUP_RECIPIENT_ENDPOINT )
    {
        ...
        for(lCount = 0; lCount < usbDeviceInstance.functionDriverCount; lCount++)
        {
            ...
            {
                if(lfunctionDriverPtr[lCount].driver->checkEndpointNumber(lfunctionDriverPtr[lCount].funcDriverIndex, endpointNumber) == USB_ERROR_OK)
                {
...
static USB_ERROR_STATUS _USB_DEVICE_InitializeFunctionDrivers( uint16_t configValue )
{
    ...
    for(count = 0; count < funcDriverCount; count++)
    {
            ...
            ( registeredFuncDrivers[count].configurationValue == configValue ) )
        {
            ...
            funcDriverIndex = registeredFuncDrivers[count].funcDriverIndex; 
            
            funcDriverInit =  registeredFuncDrivers[count].funcDriverInit;
            
            // Copy function driver details in to the module variable.
            usbDeviceInstance.functionDriver[usbDeviceInstance.functionDriverCount].funcDriverIndex = funcDriverIndex;
            ...
            result = registeredFuncDrivers[count].driver->initialize(funcDriverIndex, usbCD_Index, 0 /* Device index is always 0 */, 
                                                                     funcDriverInit, descrPointer );
...
static void _USB_DEVICE_DeInitializeAllFuncDrivers ( void )
{
    ...
    for(count = 0; count < usbDeviceInstance.functionDriverCount ; count++)
	{
    	...
    	{   // Deinitialize previously loaded function driver
            usbDeviceInstance.functionDriver[count].driver->deInitialize(funcDriverIndex);
        }
    }
...
static void _USB_DEVICE_BroadcastEventToClients( USB_DEVICE_EVENT event )
{
    ...
    for(count = 0; count < USB_DEVICE_MAX_CLIENTS; count++)
    {
        if(usbDeviceClients[count].clientState == DRV_CLIENT_STATUS_READY)
        {
            if(usbDeviceClients[count].callBackFunc != NULL)
            {
                // Pass event to application
                usbDeviceClients[count].callBackFunc(event);
            }
        }
    }
}
